...
const SortableItem = SortableElement(
    ({ type, hasRating, focusIndex, setFocusIndex, ind, name, level, updateSkill, deleteSkill }:any) => (
      <Wrapper>
        <Flex>
          <DragHandle />
          <Input
  ...
  const SortableList = SortableContainer(
    ({ type, focusIndex, setFocusIndex, hasRating, skills, updateSkill, deleteSkill }:any) => (
      <div>
        {skills?.map(({ name, level }:any, index:any) => (
          <SortableItem
            key={name}
            index={index}
            ind={index}
    ...
    return (
      <>
        <SortableList
          ...
          hasRating={hasRating}
          skills={skills}
          onSortEnd={({ oldIndex, newIndex }:any) => changeOrder(type, oldIndex, newIndex)}
          updateSkill={updateSkill}
          deleteSkill={deleteSkill}
          useDragHandle
        />
